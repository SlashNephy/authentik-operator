...
On versions where authentik's periodic cleanup is available, this processing can be disabled.
...
To prevent `helm uninstall` from removing the CRDs and losing the CRs, the CRDs carry `helm.sh/resource-policy: keep`, which can be disabled with the `crds.keep` value.
